...
import Svg, { Defs, LinearGradient, Stop, Pattern, Path, Rect, Circle } from 'react-native-svg';
import { type PatternBackgroundType, type PatternFunction } from '../Types/PatternTypes';

const createSvgPattern = (
  patternFunction: (props: PatternBackgroundType) => JSX.Element,
  options: PatternBackgroundType
): React.FC => {
  const { backgroundColor, foregroundColor, opacity = 1, spacing = 20, gradient } = options;
  ...
  return () => (
    <Svg width="100%" height="100%" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>
      <Defs>
        ...
        <Pattern id="pattern" width={spacing} height={spacing} patternUnits="userSpaceOnUse">
          {patternFunction({ ...options, foregroundColor: gradient ? 'url(#gradient)' : foregroundColor })}
        </Pattern>
      </Defs>
      <Rect width="100%" height="100%" fill={backgroundColor} fillOpacity={opacity} />
      <Rect width="100%" height="100%" fill="url(#pattern)" fillOpacity={opacity} />
    </Svg>
  );
};
...
export const circlePattern: PatternFunction = (options) => {
  ...
  return createSvgPattern(
    () => (
      <Circle cx={spacing/2} cy={spacing/2} r={spacing/3} fill={options.foregroundColor} />
    ),
    options
  );
};
...
export const dotPattern: PatternFunction = (options) => {
  ...
  return createSvgPattern(
    () => (
      <Circle cx={spacing/2} cy={spacing/2} r={spacing/10} fill={options.foregroundColor} />
    ),
    options
  );
};
...
export const boxesPattern: PatternFunction = (options) => {
  ...
  return createSvgPattern(
    () => (
      <Rect x={spacing/4} y={spacing/4} width={spacing/2} height={spacing/2} fill={options.foregroundColor} />
    ),
    options
  );
};
